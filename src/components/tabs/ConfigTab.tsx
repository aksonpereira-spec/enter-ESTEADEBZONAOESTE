...
import { Check, Building2, User, Users, Settings, Download, Shield, Sparkles } from 'lucide-react';
...
const ConfigTab = ({ onConfigChange }: ConfigTabProps) => {
  ...
  const handleBackup = async () => {
    ...
    try {
      const [alunos, turmas, mensalidades, chamadas, registros, nucleoConfig] = await Promise.all([
        supabase.from('alunos').select('*').eq('coordenador_id', coordenadorId),
        supabase.from('classes').select('*').eq('coordenador_id', coordenadorId),
        supabase.from('mensalidades').select('*').eq('coordenador_id', coordenadorId),
        supabase.from('attendance_sessions').select('*').eq('coordenador_id', coordenadorId),
        supabase.from('attendance_records').select('*'),
        supabase.from('nucleo_config').select('*').eq('coordenador_id', coordenadorId),
      ]);
      const backup = {
        exportedAt: new Date().toISOString(),
        version: '1.0',
        nucleo: form.nomeNucleo,
        data: {
          alunos: alunos.data || [],
          turmas: turmas.data || [],
      ...
      a.href = url;
      a.download = `backup-${(form.nomeNucleo || 'esteadeb').replace(/\s+/g, '-').toLowerCase()}-${new Date().toISOString().split('T')[0]}.json`;
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      {isFirstSetup && (
        <div className="content-card p-5 border-2 border-primary bg-primary/5">
          <div className="flex items-start gap-3">
            <div className="p-2 rounded-xl bg-primary/10"><Sparkles className="w-5 h-5 text-primary" /></div>
            <div>
              <p className="font-bold text-foreground">Bem-vindo ao Portal Núcleo ESTEADEB!</p>
              <p className="text-sm text-muted-foreground mt-1">Configure o nome do seu núcleo e coordenador abaixo para começar. Esses dados são exclusivos do seu acesso.</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="content-card p-6 border-l-4 border-l-emerald-500">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          Gere um arquivo JSON com todos os dados do seu núcleo: alunos, turmas, mensalidades, chamadas e registros de presença.
        </p>
      ...
      <div className="content-card p-4 bg-amber-50 border-amber-200">
        ...
        <p className="text-xs text-amber-700">
          ...
          <strong> Esposa do Coordenador</strong>.
        </p>
      </div>
    </div>
  );
